...
if test $num -eq 80
then
	CMD="taskset -c 80-159 snort -z 0  -v --rule-path /usr/local/etc/rules/ -c /usr/local/etc/snort/snort.lua --lua '$HYPERSCAN' $PCAP_LIST_OPT"
else
	cores=$((num+80))
	CMD="taskset -c 81-$cores snort -z 0  -v --rule-path /usr/local/etc/rules/ -c /usr/local/etc/snort/snort.lua --lua '$HYPERSCAN' $PCAP_LIST_OPT"
fi
